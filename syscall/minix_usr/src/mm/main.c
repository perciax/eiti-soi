...
/* gets proccess index in mproc table */

PUBLIC int do_getprocnr(){
	int in_pid = mm_in.m1_i1; 	/* read pid from MM input message */
	int proc_nr;			/* iterator for mproc table */

	/* DEBUG: printf("do_getprocnr called with message: %d\n", in_pid); */

	/* iterate for all proccesses in mproc table */
	for(proc_nr=0; proc_nr<NR_PROCS; proc_nr++){
		
		/* check if proccess is in use (flag) */
		if(mproc[proc_nr].mp_flags & IN_USE){

			/* DEBUG: printf("proc_nr = %d\tpid = %d\t", 
				proc_nr, mproc[proc_nr].mp_pid); */
			
			/* check if pid in table is equal to given one */
			if (in_pid == mproc[proc_nr].mp_pid){
				/* return index in mproc table */
				return proc_nr;
			}
		}
	}
	/* if there is no proccess of given pid return error */
	return ENOENT;
}
